...
@MainActor final class JarvisToolExecutor{
 ...
 func execute(_ step:JarvisStep) async -> String{
  ...
  case .openURL:
   guard let u=URL(string:step.argument),["http","https"].contains(u.scheme?.lowercased() ?? "") else{return "URL rejected."};await UIApplication.shared.open(u);return "Opened link."
