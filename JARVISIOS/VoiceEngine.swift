...
@MainActor final class VoiceEngine:ObservableObject{
 ...
 func start(){SFSpeechRecognizer.requestAuthorization{_ in};AVAudioSession.sharedInstance().requestRecordPermission{_ in};do{request=SFSpeechAudioBufferRecognitionRequest();guard let request else{return};let input=engine.inputNode;input.removeTap(onBus:0);input.installTap(onBus:0,bufferSize:1024,format:input.outputFormat(forBus:0)){b,_ in request.append(b)};try AVAudioSession.sharedInstance().setCategory(.playAndRecord,mode:.voiceChat,options:[.allowBluetooth]);try AVAudioSession.sharedInstance().setActive(true);engine.prepare();try engine.start();listening=true;task=recognizer?.recognitionTask(with:request){[weak self] r,e in guard let self else{return};if let r,r.isFinal{Task{@MainActor in self.onText?(r.bestTranscription);self.stop()}};if e != nil{Task{@MainActor in self.stop()}}}}catch{listening=false}}
