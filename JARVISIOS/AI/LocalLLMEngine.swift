...
@MainActor final class LocalLLMEngine: ObservableObject {
 ...
 init(models: LocalModelManager) { self.models = models }
 convenience init() { self.init(models: LocalModelManager()) }
 func generate(_ prompt: String) async throws -> String {
  guard let url = models.modelURL else { throw LocalLLMError.modelMissing }
  return try await LlamaNativeBackend(modelURL: url).generate(prompt: prompt)
 }
}
